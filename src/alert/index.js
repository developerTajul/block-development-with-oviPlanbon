 ...
 */
registerBlockType( 'lwhhdbd/alert', {
	...
	attributes : {
		...
		content : {
			type : 'string',
			default: __('Add alert box text here'),
		},
	},
	/**
	...
	edit( {className, setAttributes, attributes, isSelected } ) {
		...
		return (
			...
			<div className={`${className} alert alert-${style}`} role="alert">
	...
	save( { attributes } ) {
		return (
			<div className={`alert alert-${attributes.style}`} role="alert">
			  {attributes.content}
			</div>
		);
	},
